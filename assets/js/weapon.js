...
function getWeapon() {
  fetch('/api/weapons').then((response) => response.json())
    .then((data) => {
      Array.from($q('#weaponSelect').children).filter(c => c.value != 0).forEach(c => c.remove())
...
$qa('.weaponAction').forEach(btn => {
  ...
  btn.addEventListener('click', () => {
    ...
    else selectedValue = '/' + selectedValue
    ...
    if (btn.getAttribute('data-type') == 'delete') {
      ...
      if (btn.getAttribute('data-type') == 'submit') if ($weaponSelect.value != 0) fetchObject.method = 'PUT'
    ...
    fetch(`/api/weapons${selectedValue}`, fetchObject)
      .then((response) => {
