...
fetch(`json/${window.lang}/weaponabilities.json`).then(result => result.json()).then(data => { window.skillsInfo = data })
fetch(`json/${window.lang}/messageSkill.json`).then(result => result.json()).then(data => { window.messageSkill = data })
...
$qa('.selectChoixWeapon').forEach(select => {
	select.addEventListener('change', function () {
		...
		var option = Object.values(select.children).filter(child => { if (child.selected == true) return child })[0]
...
function modifyCollapse(option) {
	...
	if (window.globalWeapon.filter(w => w.key == value).length == 0) {
		fetch(`json/${value}.json`).then(result => result.json())
			.then(data => {
...
function setupWeapon(option) {
	...
	window.globalWeapon.filter(w => w.key == weaponKey)[0].skills.forEach(function (branch, indexbranch) {
		...
		window.globalWeapon.filter(w => w.key == weaponKey)[0].lines[indexbranch].forEach(line => {
			...
			bgSVG.innerHTML += '<line class="skillLine" x1="' + coordinates[0] + '%" y1="' + coordinates[1] + '%" x2="' + coordinates[2] + '%" y2="' + coordinates[3] + '%"/>'
		})
...
form.addEventListener('submit', function (e) {
	...
	buildObject.type = Object.values($q('#SelectType').children).filter(ch => { if (ch.selected == true) return ch })[0].value
