...
$.getJSON(`json/${window.lang}/weaponabilities.json`, function(result){ window.skillsInfo = result })
$.getJSON(`json/${window.lang}/messageSkill.json`, function(result){ window.messageSkill = result })
...
$('.selectChoixWeapon').change(function(){
	...
	if(window.selectedWeapon[collapse-1] == option.attr('value')) return
	window.selectedWeapon[collapse-1] = option.attr('value')
...
$('.resetButton').click(function(){
	var collapse = $(this).attr('collapse')
	var option = $(`.optionChoixWeapon[collapse = "${collapse}"][value = "${window.selectedWeapon[collapse-1]}"]`)
...
function modifyCollapse(option){
	...
	//lance le setup de l'arme apres avoir verifier si deja presente dans les donners, sinon ajout
		if(window.globalWeapon.filter(w => w.key == option.attr('value')).length == 0){
			$.getJSON(`json/${option.attr('value')}.json`, function(weaponInfo){
				window.globalWeapon.push(weaponInfo)
				setupWeapon(option)
			})
		}
...
function setupWeapon(option){
	...
	window.globalWeapon.filter(w => w.key == weaponKey)[0].skills.forEach(function (branch, indexbranch) {
		...
		branch.forEach(function (row, indexrow) {
			row.forEach(function(skill) {
				...
				var imgPath = 'img/'+weaponKey+'/'+window.globalWeapon.filter(w => w.key == weaponKey)[0].branchName[indexbranch]+'/'+skillkey+'.png'
				...
				skillObject.popover('enable').attr('data-bs-original-title', window.skillsInfo[skillkey]).attr('data-bs-content', skillDescription)
				...
				if(typeof skill.child == 'object' && typeof skill.parent == 'undefined'){
					...
					if(skill.active == true){
						...
						window.globalWeapon.filter(w => w.key == weaponKey)[0].selectedMainSkills.forEach(function(SelectedSkill, index){
		...
		window.globalWeapon.filter(w => w.key == weaponKey)[0].lines[indexbranch].forEach(line => {
			var coordinates = []
...
$('.skill-container').click(function(){
	...
	//reset popover
	changePopover($(this), window.skillsInfo[skillkey + '_description'])
...
function delSkill(skillObject,skill, weapon){
	...
	if(typeof skill.child == 'object' && typeof skill.parent == 'undefined'){
		...
		window.globalWeapon.filter(w => w.key == weapon.key)[0].selectedMainSkills.forEach(function(SelectedSkill, index){
...
function changeProgress(collapse, counter){
	...
	$('#pointProgresText'+collapse).html(window.messageSkill.RemainingPoint + counter)
}
